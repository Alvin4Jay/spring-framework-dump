...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
			...
			for (int i = 0; i < nl.getLength(); i++) {
				...
				if (node instanceof Element) {
					...
					if (delegate.isDefaultNamespace(ele)) {
						...
						// 解析custom namespace下面的几个元素 如<context:component-scan base-package="xxx..."/>
						// 对于component-scan：注册扫描到的@Component注解的类的BeanDefinition和@Autowired、@PreDestroy
						// 等注解的后置处理器的BeanDefinition
	 ...
	 * and registering it with the registry. 处理 <bean /> 标签定义并注册到registry
	 */
